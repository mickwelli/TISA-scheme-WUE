...
output:
  html_document:
    df_print: paged
vignette: |
  %\VignetteIndexEntry{GAMs Kiwere} %\VignetteEncoding{UTF-8} %\Vi
---
...
Kiwere_df <- readRDS("data\\GAMM_data\\Kiwere_df.rds")
...
GPP_Kiwere_space_plot_gg <- ggplot(data=(GPP_Kiwere_space_plot$data$fit%>% na.omit()), aes(x=x, y=y, z=tz))+
...
WUE_Kiwere_space_plot_gg <- ggplot(data=(WUE_Kiwere_space_plot$data$fit %>% na.omit()), aes(x=x, y=y, z=tz))+
...
ETa_Kiwere_space_plot_gg <- ggplot((data=ETa_Kiwere_space_plot$data$fit %>% na.omit()),
                                    aes(x=x, y=y, z=z))+
...
Ea_Kiwere_space_plot_gg <- ggplot(data=(Ea_Kiwere_space_plot$data$fit %>%  na.omit()), aes(x=x, y=y, z=z))+
  geom_raster(aes(fill=z)) + geom_contour(colour="black") + 
...
Ta_Kiwere_space_plot_gg <- ggplot(data=(Ta_Kiwere_space_plot$data$fit %>% na.omit()), aes(x=x, y=y, z=z))+
  geom_raster(aes(fill=z)) + geom_contour(colour="black") + 
...
GPP_Kiwere_space_plot_dat <- as.data.frame(GPP_Kiwere_space_plot$data$fit) %>% na.omit()
GPP_Kiwere_space_plot_dat$var <- c("Gross Primary Productivity")

GPP_Kiwere_space_plot_gg <- ggplot(data=GPP_Kiwere_space_plot_dat, aes(x=x, y=y, z=tz))+
  geom_raster(aes(fill=tz)) + geom_contour(colour="black") + scale_fill_viridis_c(name=expression(paste("GPP gC/m"^2)))+
  facet_grid(rows=vars(var)) +
  theme(element_blank(), axis.text = element_blank()) +coord_equal() 

GPP_ET_Kiwere_space <- ggdraw() +
  draw_plot(GPP_Kiwere_space_plot_gg, x=0.5, y=0.3,width = 0.5, height=0.35)+
  draw_plot(ET_Kiwere_oneplot, x=0, y=0,width = 0.5, height=1)#+
...
p <- ggdraw() +
  ...
  draw_plot(GPP_Kiwere_year_plot_gg, x=0, y=0.6, width=0.9, height=0.2) +
...
```{r plots}
Kiwere_GAMplot <- plot_grid(p, ncol=1, rel_heights=c(0.1, 1))
Kiwere_GAMplot
GPP_ET_Kiwere_space
